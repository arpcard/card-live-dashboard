...
def build_main_pane(df_drug_mapping: pd.DataFrame, rgi_parser: RGIParser, region_codes: GeographicRegionCodes, data: CardLiveDataLoader, world: geopandas.GeoDataFrame):
    geo_drug_classes_count = rgi_parser.geo_drug_sets_to_counts(df_drug_mapping).reset_index()
    geo_drug_classes_count = region_codes.add_region_standard_names(geo_drug_classes_count, region_column='geo_area_code')
    fig_map = choropleth_drug(geo_drug_classes_count, world)
